...
export default function DirectorDailyWagesPage() {
  const router = useRouter();
  const pathname = usePathname();

  const [me, setMe] = useState<Me | null>(null);
  const [records, setRecords] = useState<WageRecord[]>([]);
  const [summary, setSummary] = useState<DailyWagesSummary | null>(null);
  ...
  const loadPage = async (isRefresh = false) => {
    ...
    try {
      const [meData, wagesData, summaryData] = await Promise.all([
        fetchJSON("/me/"),
        fetchJSON("/payroll/daily-wages/"),
        fetchJSON("/payroll/daily-wages/summary/"),
      ]);
      ...
      setMe(currentUser);
      setRecords(extractArray<WageRecord>(wagesData));
      setSummary(summaryData as DailyWagesSummary);
    } catch (err) {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Continue with local navigation even if the request fails.
    } finally {
      router.replace("/");
    }
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Casual employee payroll overview
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                  Review casual employee wages calculated from attendance
                  for the current payroll week.
                </p>
          ...
          <section className="mt-9">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Current casual payroll position for the week.
              </p>
            </div>
          ...
          <section className="mt-9">
            <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Casual employee wages calculated from recorded attendance.
                </p>
              </div>
                    ...
                    {filteredRecords.length === 0 ? (
                      ...
                      filteredRecords.map((record) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 text-sm text-slate-700">
                            {formatCurrency(record.daily_wage)}
                          </td>
                          ...
                          <td className="px-5 py-4 text-right">
                            <button
                              ...
                              onClick={() =>
                                router.push(
                                  `/director/daily-wages/${record.id}`
                                )
                              }
